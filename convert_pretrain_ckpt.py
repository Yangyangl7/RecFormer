...
DEFAULT_LONGFORMER_CKPT = "longformer_ckpt/xlm-roberta-longformer-base-16384.bin"


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained_ckpt_path", type=str, default="pretrain_ckpt/pytorch_model.bin")
    parser.add_argument("--longformer_ckpt_path", type=str, default=DEFAULT_LONGFORMER_CKPT)
...
def main():
    ...
    recformer_output_dir = os.path.dirname(args.recformer_output_path)
    if recformer_output_dir:
        os.makedirs(recformer_output_dir, exist_ok=True)
    ...
    seqrec_output_dir = os.path.dirname(args.seqrec_output_path)
    if seqrec_output_dir:
        os.makedirs(seqrec_output_dir, exist_ok=True)
