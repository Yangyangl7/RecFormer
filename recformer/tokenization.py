...
class RecformerTokenizer:
    """Tokenizer wrapper for Recformer sequence packing logic."""
    ...
    def __getattr__(self, name):
        ...
        if tokenizer is None:
            raise AttributeError(
                f"{self.__class__.__name__} has no attribute '{name}' (tokenizer is not initialized)"
            )
    ...
    def encode_item(self, item):
        input_ids = []
        token_type_ids = []
        item = list(item.items())[: self.config.max_attr_num]
    ...
    def encode(self, items, encode_item=True):
        ...
        """
        items = items[::-1]
        items = items[: self.config.max_item_embeddings - 1]

        bos_token_id = self.bos_token_id if self.bos_token_id is not None else self.cls_token_id
        if bos_token_id is None:
            raise ValueError("Tokenizer must provide bos_token_id or cls_token_id.")
...
if __name__ == "__main__":
    ...
    model_path = "severinsimmler/xlm-roberta-longformer-base-16384"
    config = RecformerConfig.from_pretrained(model_path)
    tokenizer = RecformerTokenizer.from_pretrained(model_path, config=config)

    items = [
        {"title": "book one", "brand": "brand a", "category": "books"},
        {"title": "book two", "brand": "brand b", "category": "books"},
    ]
